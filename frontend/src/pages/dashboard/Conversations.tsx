...
import { MessageSquare, MessageCircle, MessagesSquare, Phone, Search, Inbox } from "lucide-react";
...
const Conversations = () => {
  ...
  // showResolved=true  → no server filter (return everything)
  // showResolved=false → server filter: only unresolved
  ...
  // Client-side search on lead name / phone
  ...
  return (
    <>
      ...
      <div className="rounded-lg border border-border bg-surface overflow-hidden">
        {/* Toolbar */}
        <div className="flex flex-col md:flex-row md:items-center gap-3 px-6 py-4 border-b border-border">
          ...
          <table className="w-full text-sm">
            <thead className="bg-elevated text-[hsl(var(--text-secondary))]">
              <tr className="text-left">
                ...
                <th className="px-6 py-3 text-[10px] uppercase tracking-wider font-medium">Phone</th>
            ...
            <tbody>
              {rows.map((r, i) => {
                ...
                return (
                  <tr
                    key={r.id}
                    onClick={() => { setSelected(r.id); console.log('conversation:', r.id); }}
                    className={cn(
